...
export function opacifyRaw(opacity: number, color: string): string {
  ...
  try {
    if (opacity < 0 || opacity > 100) {
      throw new Error(
        `provided opacity ${opacity} should be between 0 and 100`
      );
    }
    ...
    throw new Error(
      `provided color ${color} is neither a hex nor an rgb color`
    );
  } catch (e) {
    console.log(
      "color/utils",
      "opacifyRaw",
      `Error opacifying color ${color} with opacity ${opacity}: ${e}`
    );
  }
  return color;
}
...
function _opacifyHex(opacity: number, color: string): string {
  if (![5, 7, 9].includes(color.length)) {
    throw new Error(
      `provided color ${color} was not in hexadecimal format (e.g. #000000)`
    );
  }
  ...
  if (!validHexColor) {
    throw new Error(
      `provided color ${color} contains invalid characters, should be a valid hex (e.g. #000000)`
    );
  }
